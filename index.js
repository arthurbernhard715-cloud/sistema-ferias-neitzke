...
<div class="modal" id="editFeriaModal">
  <div class="modal-box">
    ...
    <input type="hidden" id="editFeriaColabId">
    <input type="text" id="editFeriaColab" placeholder="Colaborador" readonly>
    <input type="date" id="editFeriaInicio" onchange="calcularDiasEdit()">
    <input type="date" id="editFeriaFim" onchange="calcularDiasEdit()">
    <input type="number" id="editFeriaDias" placeholder="Dias" readonly>
    <textarea id="editFeriaObs" placeholder="Observações"></textarea>
    <div class="modal-footer">
...
async function carregarColabs() {
  ...
  colabs.forEach(c => {
    html += '<tr><td style="cursor: pointer; color: var(--primary); font-weight: 600; text-decoration: underline;" data-colab-id="' + c.id + '" data-colab-nome="' + c.nome + '" onclick="abrirHistoricoFeria(this.getAttribute(\'data-colab-id\'), this.getAttribute(\'data-colab-nome\'))">' + c.nome + '</td><td>' + (c.periodo_aquisitivo || '-') + '</td><td>' + c.dias_totais + '</td><td>' + c.dias_disponiveis + '</td><td>';
